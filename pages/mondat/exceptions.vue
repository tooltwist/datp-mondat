...
.my-archived-page
  section.section
    ...
    MondatNotification
      | Any transactions shown here are broken in some way.
      | They are not in a queue, aee not being processed, and are not sleeping.
      | They cannot proceed because it's not clear how they fit into the normal processing workflow.
      br
      br
      | DATP handles internal errors and application errors in this way, rather than simply writing an error to a log file and returning an error to the client,
      | because a complex transaction may contain multiple steps. The steps may have partially completed when the error occurred, leaving the 
      | transaction in an inconsistent state.
      br
      br
      | These transactions should be investigated and will need manual intervention in order to continue processing.
      | Hopefully you will never see errors in this page.
...
export default {
  ...
  created() {
    ...
    this.polling = setInterval(async () => {
      if (this.autoUpdate) {
        ...
        const url = `${this.$monitorEndpoint}/transactions/exception`
        console.log(`url=`, url)
  ...
  methods: {
    ...
    infiniteHandler: async function($state) {
      ...
      let url = `${this.$monitorEndpoint}/transactions/exception?page=${this.page}&pagesize=${PAGESIZE}`
