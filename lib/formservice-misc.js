...
async function getView(viewName, createIfNotFound) {
  let url = `${this.$datpEndpoint}//formservice/view/${viewName}`;
  if (createIfNotFound) {
...
async function getMapping(mappingId) {
  const url = `${this.$datpEndpoint}//formservice/mapping/${mappingId}`;
  // console.log(`url=`, url);
  const reply = await axios.get(url);
...
async function addMapping(mappingId, version, fieldName, source, converter) {
  ...
  const url = `${this.$datpEndpoint}//formservice/mapping`
  // console.log(`url=`, url)
  const params = {
    mappingId,
    version,
