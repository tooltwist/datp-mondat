  ...
  methods: {
    ...
    saveField: async function (newField) {
      ...
      if (newField._id < 0) {
        ...
        const url = `${this.$formserviceEndpoint}/field/${this.view}`
        // console.log(`url=`, url)
      ...
      if (field) {
        ...
        const url = `${this.$formserviceEndpoint}/field/${this.view}/${originalFieldName}`
        // console.log(`url=`, url)
      ...
      this.$buefy.dialog.confirm({
        ...
        onConfirm: async () => {
          ...
          if (fieldIndex >= 0) {
            ...
            const url = `${this.$formserviceEndpoint}/field/${this.view}/${name}`
            // console.log(`url=`, url);
    ...
    async drop(payload) {
      ...
      for (let i = 0; i < this.viewDef.fields.length; i++) {
        ...
        if (field.sequence !== i) {
          ...
          field.sequence = i
          const url = `${this.$formserviceEndpoint}/field/${this.view}/${field.name}`
    ...
    recursiveLearn(name, object) {
      ...
      switch (type) {
        ...
        case 'string':
          ...
          this.learnedFields.push({ alreadyExists, name, type: 'string', exampleValue: object })
          break
        ...
        case 'number':
          if (Number.isInteger(object)) {
            // Integer value
            alreadyExists = this.findFieldByName(name) != null
            this.learnedFields.push({ alreadyExists, name, type: 'integer', exampleValue: `${object}` })
          } else {
            // Floating point value
            alreadyExists = this.findFieldByName(name) != null
            this.learnedFields.push({ alreadyExists, name, type: 'float', exampleValue: `${object}` })
          }
          break
        ...
        case 'date':
          ...
          alreadyExists = this.findFieldByName(name) != null
          this.learnedFields.push({ alreadyExists, name, type: 'date', exampleValue: object })
          break
